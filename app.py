...
def load_prices():
    ...
    for asset, ticker in TICKERS.items():
        if ticker:
            df = yf.download(ticker, start=START_DATE, end=END_DATE, progress=False)
            series = df.get("Adj Close") or df.get("Close")
            if series is None:
                continue
            df_clean = pd.DataFrame(series)
            df_clean.columns = [asset]
...
# === CURRENT REGIME & ALLOC ===
latest_regime = regime_df["regime"].dropna().iloc[-1].lower()
current_alloc = allocations.get(latest_regime, {})

# === DASHBOARD HEADER ===
st.markdown("""<style>@import url('https://fonts.googleapis.com/css2?family=UnifrakturCook:wght@700&display=swap');
.gothic-title { font-family: 'UnifrakturCook', serif; text-align: center; font-size: 3.5rem; font-weight: bold; padding: 0.5rem 0; letter-spacing: 1px; }
.pub-info { text-align: center; font-family: 'Georgia', serif; font-size: 0.8rem; margin-top: -18px; color: #ccc; }</style>
<div class='gothic-title'>The Regime Report</div>
<div class='pub-info'>No. 01 · Published biWeekly · Market Bulletin · June 2025</div>
<h3 style='text-align: center; font-family: Georgia, serif; font-style: italic; margin-top: -10px;'>Asset Allocation in Current Market Conditions</h3>""", unsafe_allow_html=True)

st.markdown("""<style>.block-container { padding-left: 5vw; padding-right: 5vw; }
.section-title { font-family: Georgia, serif; font-size: 1.1rem; font-weight: bold; text-transform: uppercase; margin-bottom: 6px; color: #d4af37; border-bottom: 1px solid #555; padding-bottom: 4px; }</style>""", unsafe_allow_html=True)

# === LAYOUT ===
left_col, right_col = st.columns([1.3, 1])

with left_col:
    st.markdown("""<style>.left-section-title { font-family: Georgia, serif; font-size: 1.1rem; font-weight: bold; text-transform: uppercase; margin-bottom: 10px; text-align: center; }</style>""", unsafe_allow_html=True)
    ...
    if current_alloc:
        ...
        if filtered_alloc:
            ...
            fig_pie.update_traces(textinfo='percent', textfont_size=16, pull=[0.03]*len(filtered_alloc), marker=dict(line=dict(color="#000000", width=2)))
            fig_pie.update_layout(showlegend=False, margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_pie, use_container_width=True)
        ...
        """ + """".join([
            f"<li><strong>{asset.capitalize()}</strong>: {weight:.1%}</li>"
            for asset, weight in filtered_alloc.items()
        ]) + """
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
with right_col:
    st.markdown("""<style>
.section-title { font-family: Georgia, serif; font-size: 18px; font-weight: bold; text-transform: uppercase; margin-bottom: 6px; text-align: left; color: white; border-bottom: 1px solid #555; padding-bottom: 4px; }
.section-comment { font-family: Georgia, serif; font-size: 0.9rem; font-style: italic; color: #ccc; background-color: #262730; padding: 10px; border-radius: 5px; min-height: 130px; border: 1px solid #444; }
@media (max-width: 768px) { .section-title { font-size: 14px; } }
</style>""", unsafe_allow_html=True)

    NOTES_FILE = "thoughts.txt"
    default_sections = {"Market Insight": "", "Top Strategy Note": "", "Trader's Conclusion": ""}
    ...
    if is_admin_mode and not st.session_state.auth:
        with st.expander("\U0001f512 Admin Login (edit mode)", expanded=False):
            pwd = st.text_input("Enter password", type="password")
            if pwd == st.secrets["auth"]["edit_password"]:
                st.session_state.auth = True
                st.success("Edit mode activated!")
    ...
    for section_title in commentary:
        ...
        with cols[0]:
            st.markdown(f"<div class='section-title'>{section_title}</div>", unsafe_allow_html=True)
            if st.session_state.auth:
                commentary[section_title] = st.text_area(f"{section_title} input", value=commentary[section_title], height=130, key=section_title)
            else:
                content = commentary[section_title].strip() or "..."
                st.markdown(f"<div class='section-comment'>{content}</div>", unsafe_allow_html=True)
        st.markdown("<div style='margin-bottom: 30px;'></div>", unsafe_allow_html=True)
